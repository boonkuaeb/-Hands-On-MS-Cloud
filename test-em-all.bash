...
#
#    EXEC="docker run --rm -it --network=ecom-network alpine"
#
...
#    for ((n=0; n<3; n++))
#    do
#        assertCurl 500 "curl -k http://$HOST:$PORT/product-composite/$PROD_ID_REVS_RECS?delay=3 $AUTH -s"
...
#    assertCurl 200 "curl -k http://$HOST:$PORT/product-composite/$PROD_ID_REVS_RECS?delay=3 $AUTH -s"
...
assertCurl 200 "curl -k http://$HOST:$PORT/product-composite/$PROD_ID_NO_RECS $AUTH -s"
...
assertCurl 200 "curl -k http://$HOST:$PORT/product-composite/$PROD_ID_NO_REVS $AUTH -s"
assertEqual $PROD_ID_NO_REVS $(echo $RESPONSE | jq .productId)
assertEqual 3 $(echo $RESPONSE | jq ".recommendations | length")
assertEqual 0 $(echo $RESPONSE | jq ".reviews | length")

# Verify that a 422 (Unprocessable Entity) error is returned for a productId that is out of range (-1)
assertCurl 422 "curl -k http://$HOST:$PORT/product-composite/-1 $AUTH -s"
assertEqual "\"Invalid productId: -1\"" "$(echo $RESPONSE | jq .message)"

# Verify that a 400 (Bad Request) error error is returned for a productId that is not a number, i.e. invalid format
assertCurl 400 "curl -k http://$HOST:$PORT/product-composite/invalidProductId $AUTH -s"
...
assertCurl 403 "curl -k http://$HOST:$PORT/product-composite/$PROD_ID_REVS_RECS $READER_AUTH -X DELETE -s"
